...
class Trainer(AbstractTrainer):
    ...
    def _full_sort_batch_eval(self, batched_data):
        # interaction: 用户交互数据（不包含物品ID）
        # history_index: 用户历史交互物品的索引，用于在评估时屏蔽
        # positive_u: 正样本用户ID
        # positive_i: 正样本物品ID
        interaction, history_index, positive_u, positive_i = batched_data
        try:
            # 2. 尝试使用模型的高效全排序预测方法
            # Note: 此处的interaction不包含具体的物品ID，模型需要内部处理所有物品
        ...
        # 7. 将分数重塑为二维矩阵：[用户数 × 物品数]
        # 每行代表一个用户对所有物品的预测分数
        scores = scores.view(-1, self.tot_item_num)
        # 8. 屏蔽不需要参与排序的物品
        # 将第一个物品的分数设为负无穷（通常是填充物品或无效物品）
        scores[:, 0] = -np.inf
        
        # 9. 屏蔽用户的历史交互物品，避免推荐已交互过的物品
        if history_index is not None:
            # history_index是一个布尔张量，标记哪些用户-物品对是历史交互
            # 将这些位置的分数设为负无穷，使其在排序中不会被选中
    ...
    def evaluate(
        ...
        for batch_idx, batched_data in enumerate(iter_data):
            ...
            # 执行评估函数，获取预测分数和正样本信息
            # interaction: 交互数据, scores: 预测分数
            # positive_u: 正样本用户, positive_i: 正样本物品
